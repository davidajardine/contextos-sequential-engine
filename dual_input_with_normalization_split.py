...
class DualInputTradingDataset(Dataset):
    def __init__(self, db_path='bybit_data.db', seq_expected=288, date_range=None, eng_scaler=None, seq_scaler=None):
        """
        Loads:
          - Engineered features from training_set (one row per trading day).
          - Sequential 5-min data from custom_5min.
        If date_range is provided, filters the engineered features accordingly.
    ...
    def __getitem__(self, idx):
        day = self.dates[idx]
        day_str = str(day)  # Convert date to string for collation
...
def train_dual_model(model, dataloader, criterion, optimizer, num_epochs=50, device='cpu'):
    ...
    for epoch in range(num_epochs):
        ...
        for _, seq_input, eng_input, target in dataloader:
            ...
            loss.backward()
            # Log gradient norm (example for first layer in fc_eng)
...
#########################################
# Main Routine: Training and Testing
#########################################
def main():
    ...
    # Create DataLoaders (batch size can be adjusted)
    ...
    seq_input_dim = 7  # 5-min features: open, high, low, close, volume, turnover, open_interest
    eng_input_dim = training_dataset.df_eng.drop(columns=['date']).shape[1] - 1  # Excluding target
    seq_hidden_dim = 64
    seq_num_layers = 2
    fc_hidden_dim = 32
    ...
    trained_model = train_dual_model(model, train_loader, criterion, optimizer, num_epochs=50, device='cpu')
    ...
    # Save testing predictions to a new table in the database
    conn = sqlite3.connect(db_path)
