...
#########################################
# Custom Focal Loss (with increased gamma=4)
#########################################
...
#########################################
# Dual-Input Model Definition (Dropout Removed from FC branch)
#########################################
class DualInputModel(nn.Module):
    def __init__(self, seq_input_dim, seq_hidden_dim, seq_num_layers, eng_input_dim, fc_hidden_dim):
        ...
        # Removed dropout here for diagnostics
    ...
    def forward(self, seq_input, eng_input):
        ...
        # Log the pre-activation (before the final linear+sigmoid) for diagnostic purposes
...
def train_model(model, dataloader, criterion, optimizer, num_epochs=50, device='cpu'):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            # Also log one batch's outputs (post-Sigmoid) for diagnostic:
            with torch.no_grad():
                sample_batch = next(iter(dataloader))
                _, sample_seq, sample_eng, _ = sample_batch
                sample_out = model(sample_seq.to(device), sample_eng.to(device))
                sample_out_np = sample_out.squeeze().cpu().numpy()
                logger.info(f"Sample output probabilities at epoch {epoch+1}: {sample_out_np}")
    if epoch_summary:
...
def main():
    ...
    seq_input_dim = 7  # number of features in sequential data
    ...
    # Use weight_for_one = count_0 / count_1, but here our distribution seems extreme.
    weight_for_one = count_0 / count_1 if count_1 != 0 else 1.0
    logger.info(f"Weight for positive class: {weight_for_one:.4f}")
    
    # Use our adjusted focal loss with gamma=4
    criterion = FocalLoss(alpha=1.0, gamma=4.0, reduction='mean')
    optimizer = optim.Adam(model.parameters(), lr=0.01)  # slightly increased lr
